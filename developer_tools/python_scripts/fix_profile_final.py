          ...
          <div className="bg-[var(--hp-bg-2)] border border-[var(--hp-border)] rounded-2xl p-6 md:p-8 shadow-2xl backdrop-blur-xl mb-6 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
              ...
              <div className="flex flex-col justify-center gap-3 md:border-l md:border-[var(--hp-border)] md:pl-8">
                ...
                <div className="border-t border-[var(--hp-border)] pt-2 mt-2">
                  <h3 className="text-xs uppercase tracking-wider text-[var(--hp-text-muted)] mb-1">Daily Login Reward</h3>
                  {isDailyRewardAvailable ? (
                    <button onClick={handleClaimReward} disabled={claimingReward} className="inline-flex items-center gap-1.5 font-bold text-[var(--hp-text)] bg-[var(--hp-primary)]/10 border border-[var(--hp-primary)]/50 px-3 py-1 rounded-md w-fit hover:bg-[var(--hp-primary)]/20 transition-all text-xs cursor-pointer">
                      <Zap size={14} className="text-[var(--hp-primary)]" />
                      {claimingReward ? "Claiming..." : "Available (Claim Now)"}
                    </button>
                  ) : (
                    <div className="inline-flex items-center gap-1.5 font-bold text-[var(--hp-text-muted)] bg-[var(--hp-bg-3)] border border-[var(--hp-border)] px-3 py-1 rounded-md w-fit text-xs">
                      <CheckCircle size={14} className="text-[var(--hp-text-muted)] opacity-50" />
                      Claimed Today
                    </div>
                  )}
                </div>
...
# Ensure hooks for isDailyRewardAvailable are added
hooks_addition = """
  ...
  useEffect(() => {
    ...
    checkDailyBonusAvailable().then(available => setIsDailyRewardAvailable(available));
  }, [user]);
...
import re
if 'isDailyRewardAvailable' not in c:
    c = c.replace('import { useAuth, getUserBadgesAndRank } from "@/lib/auth";', 'import { useAuth, getUserBadgesAndRank } from "@/lib/auth";\nimport { useState, useEffect } from "react";')
    c = c.replace('const xpProgress = Math.min(\n    ((xp - CURRENT_RANK.minXP) / (NEXT_RANK.minXP - CURRENT_RANK.minXP)) * 100,\n    100\n  );', 'const xpProgress = Math.min(\n    ((xp - CURRENT_RANK.minXP) / (NEXT_RANK.minXP - CURRENT_RANK.minXP)) * 100,\n    100\n  );\n' + hooks_addition)
    
    # We replace from {/* Header Profile Card */} up to <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    c = re.sub(r'\{\/\*\s*Header Profile Card\s*\*\/\}.*?(?=<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">)', lambda m: new_header + '\n\n          ', c, flags=re.DOTALL)
    
    # Wait, the original regex didn't match before because `grid grid-cols-1 lg:grid-cols-3 gap-6` wasn't there? Let's check!
    # Ah, in `git restore`, the file was reverted. It should be there.
    
    # Global typography and wrappers fixes
    c = c.replace('text-9xl', 'text-6xl')
    c = c.replace('text-8xl', 'text-6xl')
    c = c.replace('max-w-[1440px]', 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8')
    c = c.replace('px-6 lg:px-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mx-auto', 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8')

    with open(path, 'w', encoding='utf-8') as f:
        f.write(c)
    print("Profile fixed fully!")
else:
    print("Profile already has hooks.")
