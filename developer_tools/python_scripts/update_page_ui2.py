...
# Ensure failed is false when activating
...
grid_pattern = r'<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">'
...
overlays = """
        <div className="relative w-full h-full min-h-[500px]">
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 transition-all duration-500 ${!active && !solved && !failed ? 'opacity-20 blur-md pointer-events-none select-none' : ''}`}>
"""

content = content.replace('<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">', overlays)

# Find the `</div>` that closes this grid, which is right before the `let lockedContent = null;` or `if (!unlocked) {`
# The main grid div is opened on line 281. We need to close the `.relative` div.
# Instead of doing it dynamically, we can just replace the bottom of the return statement.
# Bottom looks like:
#           </div>
#         </div>
#       </div>
...
#   );
# }
bottom_pattern = r'(\s*</div>\s*</div>\s*</div>\s*</>\s*\);\s*\})'
replacement_bottom = r'\n        </div>\1'
content = re.sub(bottom_pattern, replacement_bottom, content)
