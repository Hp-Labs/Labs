...
# 5. Replace BOTH UI text inputs with SecurePoCUploader
# There are two inputs, one in the main col and one in the side panel (if it exists)
# Let's use regex to replace all `<div className="space-y-2">...</div>` or `space-y-3` containing the input.
pattern_input1 = r'<div className="space-y-[23]">.*?<input.*?placeholder="FLAG\{\.\.\.\}".*?</button>\s*</div>'
replacement1 = """<div className="space-y-3 mt-4">
                        <SecurePoCUploader onSuccess={() => handleSubmitFlag(true)} />
                      </div>"""
content, num_subs1 = re.subn(pattern_input1, replacement1, content, flags=re.DOTALL)
print(f"Replaced flag inputs: {num_subs1}")
