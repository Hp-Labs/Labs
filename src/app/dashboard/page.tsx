    ...
    <div className="min-h-screen bg-[var(--hp-bg)]" suppressHydrationWarning>
      ...
      <div className="pt-24 pb-20 px-6 lg:px-12 max-w-[1440px] mx-auto">
        ...
        <div className="mb-8 relative overflow-hidden rounded-2xl border border-[var(--hp-primary)] bg-[var(--hp-card-bg)]">
          <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 80% at 100% 50%, var(--hp-primary), transparent)" }} />
            ...
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  ...
                  <span className="text-[var(--hp-text-muted)] opacity-50 text-xs">•</span>
                  <span className="text-xs text-[var(--hp-text-muted)]">Joined {user.joinedAt}</span>
                </div>
                {/* XP Progress bar */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-mono text-[var(--hp-text-muted)]">{DOMAIN_CATEGORIES.filter(d => d.status === "available").length} available · {DOMAIN_CATEGORIES.filter(d => d.status === "coming_soon").length} coming soon</span>
            </div>
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  <Link
                    key={href}
                    href={href}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[var(--hp-border)] hover:border-[var(--hp-border-hover)] hover:bg-[var(--hp-primary)] transition-all group"
                  >
                    <span className="text-sm">{emoji}</span>
                    <span className="text-xs text-[var(--hp-text-muted)] group-hover:text-white transition-colors flex-1">{label}</span>
                    <ChevronRight size={11} className="text-[var(--hp-text-muted)] group-hover:text-white transition-colors" />
                  </Link>
                ))}
              </div>
            </div>
  ...
  const inner = (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(135deg, ${cat.glow}, var(--hp-card-bg))`,
      }}
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {isAvailable ? (
          ...
          <span className="text-[9px] font-mono px-2 py-0.5 rounded-full border border-[var(--hp-border)] text-[var(--hp-text-muted)] bg-[var(--hp-bg-3)]">
            COMING SOON
          </span>
        )}
      </div>
